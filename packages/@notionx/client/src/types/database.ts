...
export type NormalizeDatabaseSchema<S extends DatabaseSchema> = {
  [key in keyof S]: S[key] extends PropertyConfig
    ? S[key]
    : S[key] extends PropertyType
    ? { name: key; type: S[key] }
    : never;
};

export type ExtractPropertyConfigType<T extends PropertyType | PropertyConfig> =
  T extends PropertyType ? T : T extends PropertyConfig ? T['type'] : never;
